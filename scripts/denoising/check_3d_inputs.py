"""
SourceValidate 3D raw data and sparse 2.5D U-Net annotations before processing.

The default manually annotated target slices are Z5, Z10, Z15, Z20, Z25, Z30,
and Z35. Each target is stored as one binary TIFF, for example
``results/training_files/U-net/3d_25d/<sample>/z005_mask.tif``. Only the middle
...
def main() -> int:
    ...
    parser.add_argument("--raw-only", action="store_true", help="Skip manual-mask requirements")
    parser.add_argument(
        "--out",
        type=Path,
    ...
    for zarr_path in zarrs:
        report = check_3d_sample(
            ...
            require_masks=not args.raw_only,
        )
