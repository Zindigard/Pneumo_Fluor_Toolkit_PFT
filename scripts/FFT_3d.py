...
def _import_pft_decoder(repo_root: Path):
    """
    Import the canonical decoder that returns (C,Z,Y,X).
    Falls back to adding repo_root/src to sys.path.
    """
    try:
        ...
        from PFT.core_prog_parts.decoder_omezar import decode_omezarr_volume  # type: ignore
        return decode_omezarr_volume
