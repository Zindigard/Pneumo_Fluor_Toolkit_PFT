"""Provide command-line and programmatic utilities for cellpose finetune two-dimensional data wga dapi core."""
...
@dataclass
class CellposeFineTune2DWgaDapiConfig:
    """Store validated configuration or result data for cellpose fine tune2 dwga dapi config."""
    ...
    save_every: int = 50
    model_name: str = 'cellpose_2d_wga_dapi'
    ...
    def root(self) -> Path:
        ...
        Example:
            >>> instance = CellposeFineTune2DWgaDapiConfig(...)
...
def finetune_cellpose(cfg: CellposeFineTune2DWgaDapiConfig) -> Path:
    """Return finetune cellpose for the supplied inputs.

    Args:
        cfg (CellposeFineTune2DWgaDapiConfig): Value specifying cfg for the operation.
    ...
    for img_path, mask_path, sample in pairs:
        train_images.append(_extract_2d_image(img_path, channels=(0, 1)))
