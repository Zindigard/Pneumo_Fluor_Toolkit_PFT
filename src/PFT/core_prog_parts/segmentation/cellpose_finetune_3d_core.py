"""Provide command-line and programmatic utilities for cellpose finetune three-dimensional data core."""
...
@dataclass
class CellposeFineTune3DConfig:
    """Store validated configuration or result data for cellpose fine tune3 dconfig."""
    ...
    def root(self) -> Path:
        ...
        Example:
            >>> instance = CellposeFineTune3DConfig(...)
...
def finetune_cellpose(cfg: CellposeFineTune3DConfig) -> Path:
    """Return finetune cellpose for the supplied inputs.

    Args:
        cfg (CellposeFineTune3DConfig): Value specifying cfg for the operation.
    ...
    for img_path, mask_path, sample in pairs:
        volume = _extract_3d_volume_as_slices(img_path, channels=(0, 1, 2))
        labels = _load_3d_mask(mask_path)
        if volume.shape[0] != labels.shape[0]:
            raise ValueError(f"Z mismatch for {sample}: image={volume.shape}, mask={labels.shape}")
        for z in range(volume.shape[0]):
            train_images.append(volume[z])
            train_labels.append(labels[z])
            sample_names.append(f"{sample}__z{z:03d}")
