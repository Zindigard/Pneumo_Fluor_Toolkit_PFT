...
class MinimalImageMeta:
    """Small metadata object used for TIFF input before OME-Zarr export."""
...
def resolve_omezarr_path(path: str | Path) -> Path:
    ...
    if p.is_dir():
        ...
        if len(hits) > 1:
            ...
            raise ValueError(
                "Folder contains more than one image.ome.zarr. "
                f"Please select one specific folder. First matches:\n{shown}"
            )
...
def pct_limits(image: Any, p_low: float = 1.0, p_high: float = 99.8) -> tuple[float, float] | None:
    ...
    if flat.size > max_points:
        step = max(1, flat.size // max_points)
        flat = flat[::step]
...
def save_array_as_omezarr(path: Path, arr: np.ndarray, meta: Any) -> Path | None:
    ...
    try:
        return save_ome_zarr_next_to_outputs(
            out_dir=out_dir,
            arr=arr,
            meta=meta,
            overwrite=True,
...
def read_image(path: str) -> list[LayerData]:
    ...
    if p.is_dir() and lower_name.endswith(SUPPORTED_ZARR_SUFFIXES):
        ...
        return make_layers_from_array(
            arr,
            source_path=p,
            meta=None,
            source_format="OME-Zarr",
            ome_zarr_path=resolve_omezarr_path(p),
            zarr_attrs=attrs,
        )
    ...
    if p.is_dir() and (p / "image.ome.zarr").exists():
        ...
        return make_layers_from_array(
            ...
            source_path=p,
            meta=None,
            source_format="OME-Zarr parent folder",
